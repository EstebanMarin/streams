...
trait GameDef:
  ...
  /** The target position where the block has to go. This value is left
    * abstract.
    */
  def goal: Pos
  ...
  /** In Bloxorz, we can move left, right, Up or down. These moves are encoded
    * as case objects.
    */
  enum Move:
  ...
  case class Block(b1: Pos, b2: Pos):
    ...
    /** Returns the list of blocks that can be obtained by moving the current
      ...
      */
    def neighbors: List[(Block, Move)] = Move.values.toList.map(move =>
      move match
        case Move.Down  => (this.down, move)
        case Move.Up    => (this.up, move)
        case Move.Left  => (this.left, move)
        case Move.Right => (this.right, move)
    )
    ...
    def legalNeighbors: List[(Block, Move)] =
      if this.isLegal then neighbors.filter((block, _) => block.isLegal) else Nil
